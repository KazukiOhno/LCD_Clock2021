...
#define RTC_VDSL 0x80   //0:2.1V、1:1.3Vに電圧低下検出レベル設定
#define RTC_VDET 0x40   //1:電圧低下の検出あり
#define RTC_XST  0x20   //0:発信停止の検出あり
#define RTC_PON  0x10   //1:パワーオンリセット発生の検出あり
#define BCLEN1   0x08   //1=/CLEN1
#define RTC_CTFG 0x04   //INTA割り込みがレベルモードの時、0 writeで/INTA=Offとする
#define RTC_WAFG 0x02   //WALE=1の時、アラームW発生時=1、0writeで/INTB=Off
#define RTC_DAFG 0x01   //DALE=1の時、アラームD発生時=1
...
void RTC_setTime(uint8_t *datetime) {
    ...
    I2C2_Write1ByteRegister(RX8025, 0x00, datetime[0]);
//    I2C2_Write1ByteRegister(RX8025, 0x00, 0x00); //秒は00とする
    __delay_us(80); //Stopから次のStartまで、62us以上
...
uint8_t getWeekdays(uint8_t *year, uint8_t *month, uint8_t *day) {
    ...
    *year = t2->tm_year - 100;  //1900年基準を2000年基準に
    *month = (t2->tm_mon) + 1; //適切な月日に。4/31→5/1とか
    *day = t2->tm_mday;

    return t2->tm_wday; //曜日を返す
}
