...
# 52 "C:/Program Files/Microchip/MPLABX/v5.50/packs/Microchip/PIC18F-K_DFP/1.4.87/xc8\pic\include\proc\pic18f26k22.h"
...
extern volatile __bit CCP4 __at(0x7C08);
...
extern volatile __bit CCP5 __at(0x7C04);
...
extern volatile __bit CK2 __at(0x7C0E);
...
extern volatile __bit DT2 __at(0x7C0F);
...
extern volatile __bit NOT_SS2 __at(0x7C08);
...
extern volatile __bit P3B __at(0x7C17);
...
extern volatile __bit RX2 __at(0x7C0F);
...
extern volatile __bit SCK2 __at(0x7C09);
...
extern volatile __bit SCL2 __at(0x7C09);
...
extern volatile __bit SDA2 __at(0x7C0A);
...
extern volatile __bit SDI2 __at(0x7C0A);
...
extern volatile __bit SDO2 __at(0x7C0B);
...
extern volatile __bit SS2 __at(0x7C08);
...
extern volatile __bit TX2 __at(0x7C0E);
...
extern volatile __bit nSS2 __at(0x7C08);
...
0x00, 0xB0, 0xF0, 0x70, 0x00, 0x00, 0x00, 0x00,
...
0x15, 0x16, 0x7C, 0x7C, 0x16, 0x15, 0x00, 0x00,
...
0x7C, 0x7C, 0x18, 0x0C, 0x0C, 0x0C, 0x00, 0x00,
0x48, 0x5C, 0x54, 0x54, 0x74, 0x24, 0x00, 0x00,
0x00, 0x08, 0x3C, 0x7C, 0x48, 0x00, 0x00, 0x00,
0x3C, 0x7C, 0x40, 0x40, 0x7C, 0x7C, 0x00, 0x00,
0x0C, 0x38, 0x60, 0x30, 0x0C, 0x00, 0x00, 0x00,
0x3C, 0x70, 0x3C, 0x70, 0x3C, 0x00, 0x00, 0x00,
0x44, 0x6C, 0x38, 0x38, 0x6C, 0x44, 0x00, 0x00,
0x0C, 0x18, 0x70, 0x18, 0x0C, 0x00, 0x00, 0x00,
...
0x40, 0x3F, 0x15, 0x15, 0x55, 0x7F, 0x00, 0x00,
0x00, 0x46, 0x30, 0x0F, 0x30, 0x46, 0x00, 0x00,
0x24, 0x14, 0x0C, 0x7F, 0x0C, 0x12, 0x21, 0x00,
0x22, 0x12, 0x0A, 0x7F, 0x0A, 0x12, 0x22, 0x00,
0x54, 0x72, 0x55, 0x7D, 0x55, 0x72, 0x54, 0x00,
0x40, 0x44, 0x44, 0x7F, 0x44, 0x44, 0x40, 0x00,
...
0x00,0x02,0x7f,0x7f,0x00,0x00,
0x66,0x73,0x59,0x4f,0x46,0x00,
0x22,0x6b,0x49,0x7f,0x36,0x00,
0x1c,0x13,0x7f,0x7f,0x10,0x00,
0x2f,0x47,0x45,0x7d,0x39,0x00,
0x38,0x7e,0x4b,0x79,0x38,0x00,
0x01,0x61,0x79,0x1f,0x07,0x00,
0x36,0x7f,0x49,0x7f,0x36,0x00,
0x06,0x4f,0x69,0x7f,0x1e,0x00,
...
# 210
touch_status = (unsigned)(touch_status << 1) | PORTAbits.RA4;
...
# 233
slideSW_status = (unsigned)(slideSW_status << 1) | PORTAbits.RA7;
...
if (Count1PPS < 20) {
G1PPS_status = (unsigned)(G1PPS_status << 1) | PORTBbits.RB5;
...
static int8_t Bdata = 0;
static uint8_t RotA_status = 0xff;
uint8_t newBdata;


RotA_status = (unsigned)(RotA_status << 1) | PORTCbits.RC1;
...
for (jj = 0; jj < 4; jj++) {
tmp[jj] = (unsigned)DATAEE_ReadByte(0x08 + jj);
if (tmp[jj] != DateTime[jj+3]) {
DATAEE_WriteByte(0x08 + jj, DateTime[jj+3]);
UpdateFlag |= 0x20;
}
}
}
...
if (f_open(&file, "TempLog.TXT", 0x02 | 0x30 ) == FR_OK) {
...
IncDecTime(SmoothCount*5, &hh, &mm);
...
if (ButtonPush(Test_x, Test_y, 6)) {
DisplayMode = (DisplayMode +1) % 3;
DATAEE_WriteByte(0x07, DisplayMode);
FirstDraw = 1;

Mode = 0x00;

ButtonObj3[0] = RYear[DisplayMode];
ButtonObj3[1] = RMonth[DisplayMode];
ButtonObj3[2] = RDay[DisplayMode];
ButtonObj3[3] = RTime[DisplayMode];
ButtonObj3[6] = RPrevMonthCalendar[DisplayMode];
...
mm = Bcd2Hex(AlarmTime[0]);
if (DisplayMode == 0x02) mm = mm - (mm % 5);
hh = Bcd2Hex(AlarmTime[1]);
...
if (GetGPS >= 1) {
display_fillCircle(7, 230, 3, 0xF800);
} else {
display_fillCircle(7, 230, 3, 0x0400);
}
...
display_drawChars(RDate[DisplayMode].x, RDate[DisplayMode].y, str, 0xFFFF, 0x0000, RDate[DisplayMode].font);

if (mode == 0x11) {
sprintf(str, "20%02x", TmpTime[6]);
display_drawChars(RYear[DisplayMode].x, RYear[DisplayMode].y, str, 0xF800, 0x0000, RDate[DisplayMode].font);
}
else if (mode == 0x12) {
sprintf(str, "%02x", TmpTime[5]);
display_drawChars(RMonth[DisplayMode].x, RMonth[DisplayMode].y, str, 0xF800, 0x0000, RDate[DisplayMode].font);
}
else if (mode ==0x13) {
sprintf(str, "%02x", TmpTime[4]);
display_drawChars(RDay[DisplayMode].x, RDay[DisplayMode].y, str, 0xF800, 0x0000, RDate[DisplayMode].font);
}
AlarmSoundOff();
}
...
for (jj=4; jj<=5; jj++) {
display_drawRoundRect(ButtonObj3[jj].x, ButtonObj3[jj].y, ButtonObj3[jj].xw, ButtonObj3[jj].yw, 10, 0xFFFF);
...
display_drawChars(xp, yp, (char *) ButtonName[jj], 0xFFFF, 0x0000, jj-3);
}
...
DateTime[0] = 0x00;
if (changeTime) RTC_setTime(DateTime);
...
drawTime(DisplayMode, TmpTime, 0xF800);

changeTime = 1;
} else if (Mode >= 0x11 && Mode <= 0x13) {
yy = Bcd2Hex(TmpTime[6]);
mo = Bcd2Hex(TmpTime[5]);
dd = Bcd2Hex(TmpTime[4]);

if (Mode == 0x11) {
yy = (yy + delta -1 + 99) % 99 +1;
} else if (Mode == 0x12) {
...
for (jj=0; jj<3; jj++) {
DATAEE_WriteByte(0x0c+jj, AlarmTime[jj]);
}
SlideSWStatus++;
...
start++;
while (start < end) {
checksum = (uint8_t)(checksum ^ (*start));
start++;
}
...
# 918
DateTime[2] = ((Buffer[7]-'0')<<4) + (Buffer[8]-'0');
DateTime[1] = ((Buffer[9]-'0')<<4) + (Buffer[10]-'0');
DateTime[0] = ((Buffer[11]-'0')<<4) + (Buffer[12]-'0');
...
DateTime[4] = ((p[1]-'0')<<4) + (p[2]-'0');
DateTime[5] = ((p[3]-'0')<<4) + (p[4]-'0');
DateTime[6] = ((p[5]-'0')<<4) + (p[6]-'0');
...
display_drawChars(50, 100, str, color1, 0x0000, 1);
sprintf(str, "2. Touch + at Bottom Right");
display_drawChars(50, 120, str, color2, 0x0000, 1);
}
...
display_drawLine(17, 20, 23, 20, 0xFFFF);
display_drawLine(20, 17, 20, 23, 0xFFFF);
...
display_drawLine(17, 20, 23, 20, 0x8410);
display_drawLine(20, 17, 20, 23, 0x8410);


display_drawLine(297, 220, 303, 220, 0xFFFF);
display_drawLine(300, 217, 300, 223, 0xFFFF);
...
IOCB4_SetInterruptHandler(AlarmWHandler);
...
init_RTC(DateTime);
INT0_SetInterruptHandler(RTC_handler);
...
DATAEE_WriteByte(0x18, ((T_x1) & 0xff));
DATAEE_WriteByte(0x18+1, ((T_x1) >> 8));
DATAEE_WriteByte(0x18+2, ((T_y1) & 0xff));
DATAEE_WriteByte(0x18+3, ((T_y1) >> 8));
DATAEE_WriteByte(0x18+4, ((T_x2) & 0xff));
DATAEE_WriteByte(0x18+5, ((T_x2) >> 8));
DATAEE_WriteByte(0x18+6, ((T_y2) & 0xff));
DATAEE_WriteByte(0x18+7, ((T_y2) >> 8));
...
T_x1 = DATAEE_ReadByte(0x18) + (DATAEE_ReadByte(0x18+1)<<8);
T_y1 = DATAEE_ReadByte(0x18+2) + (DATAEE_ReadByte(0x18+3)<<8);
T_x2 = DATAEE_ReadByte(0x18+4) + (DATAEE_ReadByte(0x18+5)<<8);
T_y2 = DATAEE_ReadByte(0x18+6) + (DATAEE_ReadByte(0x18+7)<<8);




for (jj=0; jj<4; jj++) {
DateTime[jj+3] = DATAEE_ReadByte(0x08+jj);
}
for (jj=0; jj<3; jj++) {
AlarmTime[jj] = DATAEE_ReadByte(0x0c+jj);
}

DisplayMode = DATAEE_ReadByte(0x07);
}
...
# 1289
if (Mode <= 0x01) NormalProc();
else if (Mode & 0x10) SettingProc();
