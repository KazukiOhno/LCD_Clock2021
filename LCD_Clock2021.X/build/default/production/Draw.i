...
fillRect(xd, yd-2, 20*size, 27*size+4, 0x0000);
...
fillRect(xd, yd-2, 6*size+1, 27*size+4, 0x0000);
...
void DrawAnalogClock(uint8_t mode, uint8_t * datetime, uint16_t xx, uint16_t yy, uint8_t size, uint16_t color) {
uint16_t xc, yc;
uint16_t rc, rc1, rcs, rcm, rch, rca;
uint16_t rc2;
uint16_t rcs2, rcm2, rcm3, rch2, rch3, rca2;
uint16_t x[5], y[5];
uint16_t temp, angle;
...
yc = yy;
rc = size;
display_drawCircle(xc, yc, rc+3, color);
display_drawCircle(xc, yc, rc, color);
...
display_drawLine(pax[0], pay[0], pax[1], pay[1], 0x0000);
if ( ((pay[1]-pay[0]) < 0 ? -(pay[1]-pay[0]) : (pay[1]-pay[0])) > ((pax[1]-pax[0]) < 0 ? -(pax[1]-pax[0]) : (pax[1]-pax[0]))) {
display_drawLine(pax[0]+1, pay[0], pax[1]+1, pay[1], 0x0000);
display_drawLine(pax[0]-1, pay[0], pax[1]-1, pay[1], 0x0000);
} else {
display_drawLine(pax[0], pay[0]+1, pax[1], pay[1]+1, 0x0000);
display_drawLine(pax[0], pay[0]-1, pax[1], pay[1]-1, 0x0000);
}
}
...
display_fillTriangle(phx[1], phy[1], phx[2], phy[2], phx[3], phy[3], 0x0000);
display_drawTriangle(phx[1], phy[1], phx[2], phy[2], phx[3], phy[3], 0x0000);
display_fillTriangle(phx[4], phy[4], phx[2], phy[2], phx[3], phy[3], 0x0000);
display_drawTriangle(phx[4], phy[4], phx[2], phy[2], phx[3], phy[3], 0x0000);
...
display_fillTriangle(pmx[1], pmy[1], pmx[2], pmy[2], pmx[3], pmy[3], 0x0000);
display_drawTriangle(pmx[1], pmy[1], pmx[2], pmy[2], pmx[3], pmy[3], 0x0000);
display_fillTriangle(pmx[4], pmy[4], pmx[2], pmy[2], pmx[3], pmy[3], 0x0000);
display_drawTriangle(pmx[4], pmy[4], pmx[2], pmy[2], pmx[3], pmy[3], 0x0000);
minupdate = 1;
...
ss = ((datetime[0]>>4)*10 + (datetime[0] & 0x0f));
rcs = rc1 - 4;
angle = ss*6;
csx[0] = xc;
csy[0] = yc;
...
display_drawLine(psx[1], psy[1], psx[2], psy[2], 0x0000);

display_drawLine(psx[0], psy[0], psx[3], psy[3], 0x0000);
display_drawLine(psx[0], psy[0], psx[4], psy[4], 0x0000);
...
if (jj%5 == 0) {
display_fillCircle(x[2], y[2], 2, color);
...
display_drawLine(phx[1], phy[1], phx[2], phy[2], 0xFFFF);
display_drawLine(phx[1], phy[1], phx[3], phy[3], 0xFFFF);
display_drawLine(phx[4], phy[4], phx[2], phy[2], 0xFFFF);
display_drawLine(phx[4], phy[4], phx[3], phy[3], 0xFFFF);


display_fillTriangle(pmx[1], pmy[1], pmx[2], pmy[2], pmx[3], pmy[3], mcolor);
display_drawTriangle(pmx[1], pmy[1], pmx[2], pmy[2], pmx[3], pmy[3], 0xFFFF);

display_fillTriangle(pmx[4], pmy[4], pmx[2], pmy[2], pmx[3], pmy[3], mcolor);
display_drawTriangle(pmx[4], pmy[4], pmx[2], pmy[2], pmx[3], pmy[3], 0xFFFF);

display_drawLine(pmx[1], pmy[1], pmx[2], pmy[2], 0xFFFF);
display_drawLine(pmx[1], pmy[1], pmx[3], pmy[3], 0xFFFF);
display_drawLine(pmx[4], pmy[4], pmx[2], phy[2], 0xFFFF);
display_drawLine(phx[4], phy[4], phx[3], phy[3], 0xFFFF);
...
fontw = Font.xsize * 2;
space = 4;
fonth = Font.ysize*2 +2;
ypitch = fonth+4;

yr = year;
mm = month;
dd = 1;
wd = getWeekdays(&yr, &mm, &dd);

startday = 1- wd;


dd = 31;
getWeekdays(&yr, &mm, &dd);
...
if (jj==0) color = 0xF800;
else if (jj==6) color = 0x001F;
else color = 0xFFFF;
...
display_drawChars(xx, yy, str, color, backColor, 2);
display_drawRect(xx-1, yy-2, fontw*2+2, fonth+2, 0xFFFF);
}
else {
display_drawChars(xx, yy, str, color, backColor, 2);
}
...
display_drawChars(xx, yy, str, color, backColor, 1);
display_drawRect(xx-1, yy-2, 14, 11, 0xFFFF);
}
else {
display_drawChars(xx, yy, str, color, backColor, 1);
}
...
if (mode == 0x02) {
DrawAnalogClock(mode, DateTime, RTime[mode].x, RTime[mode].y, RTime[mode].xw, 0x8410);
if (alarmtime[1] >= 0x12) ap= 1 ;
else ap = 0;
if (SlideSWStatus>>1)
...
display_drawChars(RAlarm[mode].x, RAlarm[mode].y, str, AlarmColor[SlideSWStatus>>1], 0x0000, RAlarm[mode].font);
} else {
if (SlideSWStatus>>1)
...
display_drawChars(RDate[mode].x, RDate[mode].y, str, 0xFFFF, 0x0000, RDate[mode].font);
drawTime(mode, datetime, 0x0400);
}
...
display_drawChars(RTemp[mode].x, RTemp[mode].y, str1, 0xFFFF, 0x0000, RTemp[mode].font);
display_drawChars(RHumidity[mode].x, RHumidity[mode].y, str2, 0xFFFF, 0x0000, RHumidity[mode].font);
